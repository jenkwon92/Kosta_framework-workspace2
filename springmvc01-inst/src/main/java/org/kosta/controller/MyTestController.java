...
public class MyTestController {
	...
	public String redirectTest(CustomerDTO customer) {
		...
		//아래 방식은 클라이언트(브라우저) 가 직접 jsp 에 접근할 수 없도록 구조(WEB-INF 아래) 을 정의했으므로 error
		//이유는 Front Controller Pattern 모든 클라이언트의 요청은 DispatchServlet 을 통하도록 해야하므로
		//return "redirect:result5.jsp";
		return "redirect:testResult.do?customerId="+customer.getId(); //springmvc에서 redirect 방식으로 응답할 떄는 서두에 redirect: 를 명시한다
	}
	
	@RequestMapping("restResult.do")
